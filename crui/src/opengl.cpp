...
void shader::set_uniform(const std::string& name, const uniform_value& value) const {
    ...
    std::visit(util::overloaded{
                       ...
                       [&](const std::shared_ptr<texture>& v) {
                           int texture_unit = get_texture_unit(name);
...
int shader::get_texture_unit(const std::string& name) const {
    auto it = texture_units.find(name);
    if (it != texture_units.end()) {
        return it->second;
    }
    int texture_unit = int(texture_units.size());
    texture_units[name] = texture_unit;
    return texture_unit;
}
...
void vertex_buffer::set_data(const void* data, unsigned int size, bool dyn_draw) const {
    glBindBuffer(GL_ARRAY_BUFFER, id);
    glBufferData(GL_ARRAY_BUFFER, size, data, dyn_draw ? GL_DYNAMIC_DRAW : GL_STATIC_DRAW);
}
...
unsigned int vertex_array::add_attribute(unsigned int index, vertex_array::attribute_type type, int count,
                                         unsigned int offset, unsigned int stride) const {
    glBindVertexArray(id);
    glEnableVertexAttribArray(index);
    switch (type) {
        case attribute_type::float_type:
            glVertexAttribPointer(index, count, GL_FLOAT, GL_FALSE, stride, (void*) (intptr_t) offset);
            return offset + sizeof(float) * count;
        case attribute_type::int_type:
            glVertexAttribIPointer(index, count, GL_UNSIGNED_INT, stride, (void*) (intptr_t) offset);
            return offset + sizeof(int) * count;
        case attribute_type::byte_type:
            glVertexAttribIPointer(index, count, GL_UNSIGNED_BYTE, stride, (void*) (intptr_t) offset);
...
void vertex_array::use() const {
    ...
    if (vertexBuffer) glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer->id);
    if (indexBuffer) glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBuffer->id);
}
...
cr::math::square_matrix<float, 3> opengl_window_to_pixel(int width, int height) {
    return cr::math::translate_matrix<float>(-1, 1) *
           cr::math::scale_matrix<float>(cr::math::with_translation, 2.0f / (float) width, -2.0f / (float) height);
}
...
void texture::set_data(const void* data, unsigned int width, unsigned int height, unsigned int channels) const {
    glBindTexture(GL_TEXTURE_2D, id);
    GLint format;
    switch (channels) {
        case 1:
